...
### 代替案10: 倍率の上限を 200 にする

**概要**: Inkdrop 既定より濃くもできるよう、上限を 200 とする。

**メリット**:
- エディタだけ Inkdrop 既定より不透明にして可読性を上げられる

**デメリット・不採用理由**:
- 実機での確認から実用域は 0〜60 に集中しており、100 を超える範囲はほぼ使われない
- 使われない範囲が可動域の半分を占めると、実用域の調整が細かくできない
- 「エディタだけ読みやすく」という要求は、他の領域を下げてエディタを高めに保つことで満たせる
