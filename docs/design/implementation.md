...
| 層               | 技術                        | バージョン | 選定理由                                             |
| ---------------- | --------------------------- | ---------- | ---------------------------------------------------- |
| 実行環境         | Inkdrop                     | `^6.0.0`   | 透過に使う acrylic ウィンドウと背景色変数が Inkdrop 6 の仕組みのため。v4 / v5 は `setOpacity` 前提の 1.2.x が最終版となる |
...
| 無効化                   | `alt-cmd-t` またはメニューの Deactivate で Inkdrop 既定の背景に戻ること |
| 設定変更の即時反映       | 透過中に設定値を変更し、メインウィンドウに戻ると反映されていること     |
| 既定との一致             | 設定 100 で Inkdrop 既定（プラグイン無効時）と見た目が変わらないこと   |
| 透過の強さ               | 設定 40 で明確に背後が透けること                                       |
| 一様な透過               | サイドバー・ノート一覧・エディタが同じ度合いで透過し、まだらにならないこと |
| オーバーレイ             | ダイアログや通知などの重ね表示も同じ度合いで透過すること               |
| 可読性                   | 設定を下げた際に文字が読めなくなる下限を把握しておくこと               |
| ライト／ダーク双方       | テーマを切り替えても背景が破綻しないこと                               |
| プラグイン無効化         | プラグインを無効にすると既定の背景に戻ること                           |
