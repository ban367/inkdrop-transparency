...
| 既定値の無害性           | すべて 100 のとき、プラグイン無効時と見た目が変わらないこと                 |
| 領域ごとの調整           | 1つの領域だけ下げると、その領域だけ透けること                               |
| 上限の丸め               | 200 にしても表示が破綻しないこと                                            |
| 起動時の自動適用         | Inkdrop を再起動し、設定どおりの見た目になること                            |
| 有効化                   | `ctrl-alt-cmd-t` またはメニューの Activate で適用されること                 |
| 無効化                   | `alt-cmd-t` またはメニューの Deactivate で Inkdrop 既定に戻ること           |
